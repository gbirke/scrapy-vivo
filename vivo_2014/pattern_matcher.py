...
class DateMatcher(object):
    patterns = {
        ...
        "YEAR":  "\d{4}",
        "SEMESTER": "(?:SS\s+%{YEAR}|WS\s+%{YEAR}\s*/\s*%{YEAR})",
        "YEARONLY": "%{YEAR}",
        "BASIC_NUMERIC": "%{DAY}\.\s*%{MONTH_NUMBER}%{VARIABLE_DATE_SEPARATOR}%{YEAR}",
        "BASIC_ALPHANUMERIC": "%{DAY}\.?\s*%{MONTH_NAMES}\s+%{YEAR}",
        "PERIOD_DAYS": "%{DAY}\.?\s*-\s*%{DAY}%{VARIABLE_DATE_SEPARATOR}(?:%{MONTH_NAMES}\s+|%{MONTH_NUMBER}%{VARIABLE_DATE_SEPARATOR})%{YEAR}",
        "PERIOD_MONTHS": "%{DAY}%{VARIABLE_DATE_SEPARATOR}%{MONTH_NUMBER}%{VARIABLE_DATE_SEPARATOR}-\s*%{DAY}%{VARIABLE_DATE_SEPARATOR}%{MONTH_NUMBER}%{VARIABLE_DATE_SEPARATOR}%{YEAR}",
